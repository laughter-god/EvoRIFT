...
class FeatureJitter(nn.Module):
    ...
    def forward(self, feature_tokens):
        if self.training:
            ...
            # 生成乘性噪声因子，使用对数正态分布确保正值
            # 均值为1，标准差为scale，保持特征的大致尺度
            multiplicative_factor = torch.randn((batch_size, num_tokens, dim_channel)).to(feature_tokens.device)
            multiplicative_factor = 1.0 + multiplicative_factor * self.scale
            # 应用乘性噪声
            #feature_tokens = feature_tokens * multiplicative_factor、
            if self.training:
            # 自己生成和特征维度一模一样的高斯噪声，0.1 是噪声强度 (std)
                noise = torch.randn_like(feature_tokens) * 0.1  
                
                alpha = 0.8  # 乘性权重：继续用强大的边缘破坏力对付高亮区域
                beta = 0.2   # 加性权重：强制在物体内部(哪怕激活是0)铺一层底噪
                
                # 打破恒等映射的无敌公式：
                feature_tokens = feature_tokens * (1.0 + alpha * noise) + beta * noise
