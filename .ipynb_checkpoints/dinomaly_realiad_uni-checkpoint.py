...
import argparse
from utils import evaluation_batch, global_cosine_hm_percent, regional_cosine_focal, WarmCosineScheduler
...
def get_logger(name, save_path=None, level='INFO'):
    ...
    if save_path is not None:
        ...
        file_handler = logging.FileHandler(os.path.join(save_path, 'realiad.txt'))
...
def train(item_list):
    ...
    for i, item in enumerate(item_list):
        train_data = RealIADDataset(
            root=args.data_path, category=item, transform=data_transform,
    ...
    if dino_aux_encoder_global is not None:
    ...
    for epoch in range(total_epochs):
        ...
        pbar = tqdm(
            train_dataloader, desc=f'Epoch [{current_epoch}/{total_epochs}]',
        ...
        if current_epoch % eval_interval == 0 or current_epoch == total_epochs:
            evaluate(model, item_list, train_data_list, test_data_list, batch_size)

    return


def evaluate(model, item_list, train_data_list, test_data_list, batch_size):
    print_fn('\n' + '=' * 15 + ' start performance evaluation ' + '=' * 15)
    ...
    for item, train_data_for_item, test_data in zip(item_list, train_data_list, test_data_list):
        ...
        if dino_aux_encoder_global is not None and args.dino_aux_weight > 0:
            dino_bank = build_dino_image_bank(dino_aux_encoder_global, aux_train_dataloader, device)

            def dino_score_fn(img, bank=dino_bank):
                with torch.no_grad():
                    feat = extract_dino_image_features(dino_aux_encoder_global, img)
                    return 1.0 - torch.matmul(feat, bank.t()).max(dim=1)[0]

            aux_score_parts.append((args.dino_aux_weight, dino_score_fn))

        if dino_aux_encoder_global is not None and (args.dino_patch_weight > 0 or args.dino_pixel_weight > 0):
            dino_patch_bank = build_dino_patch_bank(
                dino_aux_encoder_global, aux_train_dataloader, device,
            ...
            if args.dino_patch_weight > 0:
                ...
                dino_patch_stats = build_dino_patch_score_stats(
                    dino_aux_encoder_global, aux_train_dataloader, dino_patch_bank, device, dino_patch_ratios)
                ...
                def dino_patch_score_fn(img, bank=dino_patch_bank, stats=dino_patch_stats,
                    ...
                    with torch.no_grad():
                        return calibrated_dino_patch_score(
                            dino_aux_encoder_global, img, bank, stats, ratios)
                ...
                def aux_pixel_map_fn(img, bank=dino_patch_bank):
                    with torch.no_grad():
                        return dino_patch_distance_map(
                            dino_aux_encoder_global, img, bank, out_size=img.shape[-1])
        ...
        candidate_class = args.realiad_candidate_class if args.realiad_candidate_class else item
        ...
        print_fn('  -> {}: I-Auroc:{:.4f}, I-AP:{:.4f}, I-F1:{:.4f} | P-AUROC:{:.4f}, P-AP:{:.4f}, P-F1:{:.4f}, P-AUPRO:{:.4f}'.format(
            item.ljust(18), auroc_sp, ap_sp, f1_sp, auroc_px, ap_px, f1_px, aupro_px))
...
dino_aux_encoder_global = None


if __name__ == '__main__':
    os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--data_path', type=str, default='/root/autodl-tmp/Real')
    parser.add_argument('--save_dir', type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str, default='realiad')
    parser.add_argument('--sam_checkpoint', type=str, default='./sam_vit_b_01ec64.pth')
    parser.add_argument('--batch_size', type=int, default=8)
    parser.add_argument('--image_size', type=int, default=448)
    parser.add_argument('--crop_size', type=int, default=392)
    parser.add_argument('--total_epochs', type=int, default=24)
    parser.add_argument('--eval_interval', type=int, default=12)
    ...
    parser.add_argument('--realiad_candidate_class', type=str, default='')
    ...
    if args.dino_aux_weight > 0 or args.dino_patch_weight > 0 or args.dino_pixel_weight > 0:
        dino_aux_encoder_global = vit_encoder.load(args.dino_aux_encoder).to(device)
        dino_aux_encoder_global.eval()
        for param in dino_aux_encoder_global.parameters():
