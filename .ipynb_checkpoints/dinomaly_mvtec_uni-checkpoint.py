...
def dino_patch_normality_score(encoder, img, patch_bank, top_ratio=0.01, chunk_size=2048):
    patches = extract_dino_patch_features(encoder, img)
    ...
    return torch.topk(distances, k=k, dim=1).values.mean(dim=1)
...
def dino_patch_distance_map(encoder, img, patch_bank, out_size, chunk_size=2048):
    ...
    distance_map = F.interpolate(distance_map, size=(out_size, out_size),
...
def build_pde_residual_range(model, dataloader, device, low_q=0.05, high_q=0.95):
    model.eval()
    score_list = []
    with torch.no_grad():
        for img, _ in dataloader:
            img = img.to(device)
            score_list.append(model.pde_residual_energy(img).detach().cpu())
    if not score_list:
        return None
    scores = torch.cat(score_list, dim=0)
    std = scores.std(unbiased=False).clamp_min(1e-6)
    return {
        'low': torch.quantile(scores, low_q).to(device),
        'high': torch.quantile(scores, high_q).to(device),
        'std': std.to(device),
    }


def pde_range_score(model, img, pde_range):
    energy = model.pde_residual_energy(img)
    below = (pde_range['low'] - energy) / pde_range['std']
    above = (energy - pde_range['high']) / pde_range['std']
    return torch.relu(torch.maximum(below, above))
...
def train(item_list):
   ...
   # trainable = nn.ModuleList([bottleneck, decoder, model.defect_injector])
    ...
    if args.pde_gate_weight > 0:
        print_fn('PDE residual range gate enabled, weight={}, q=[{}, {}]'.format(
            args.pde_gate_weight, args.pde_gate_low_q, args.pde_gate_high_q))
           ...
           # p = min(p_adaptive * it / 1000, p_adaptive)
            ...
            with autocast(enabled=use_amp):
                ...
                if args.regional_focal_weight > 0:
                    loss_regional = regional_cosine_focal(
                        en, de, p=args.regional_focal_p, alpha=args.regional_focal_alpha)
        ...
        if current_epoch % eval_interval == 0 or current_epoch == total_epochs:
            ...
            for item, train_data_for_item, test_data in zip(item_list, train_data_list, test_data_list):
                ...
                results = evaluation_batch(
                    model, test_dataloader, device, _class_=item, max_ratio=args.eval_max_ratio,
                    resize_mask=args.eval_resize_mask,
                    image_score_mode='auto', aux_score_fn=aux_score_fn, aux_weight=1.0,
                    pixel_refine=args.pixel_refine, pde_map_weight=args.pde_map_weight,
                    eval_tta=args.eval_tta, pixel_score_mode=args.pixel_score_mode,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/root/autodl-tmp/MVTecAD/data/mvtec')
    ...
    parser.add_argument('--dino_patch_top_ratio', type=float, default=0.01)
    ...
    parser.add_argument('--pde_gate_low_q', type=float, default=0.05)
    parser.add_argument('--pde_gate_high_q', type=float, default=0.95)
    ...
    parser.add_argument('--p_min', type=float, default=0.8)
    parser.add_argument('--p_max', type=float, default=0.95)
    parser.add_argument('--ssim_weight', type=float, default=0.06)
    parser.add_argument('--regional_focal_weight', type=float, default=0.03)
    parser.add_argument('--regional_focal_p', type=float, default=0.985)
