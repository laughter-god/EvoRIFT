...
def get_logger(name, save_path=None, level='INFO'):
    ...
    if not save_path is None:
        ...
        fileHandler = logging.FileHandler(os.path.join(save_path, 'btad.txt'))
...
def cross_sample_energy_consistency(residual_energy_fn, label, memory):
...
# ==========================================
           ...
           # p = min(p_adaptive * it / 1000, p_adaptive)
            ...
            loss_cross = cross_sample_energy_consistency(
                model.latent_injector.residual_energy, label, cross_energy_memory)
            loss = loss_cos + args.ssim_weight * loss_ssim + lambda_cross * loss_cross
        ...
        if current_epoch % eval_interval == 0 or current_epoch == total_epochs:
            ...
            for item, train_data_for_item, test_data in zip(item_list, train_data_list, test_data_list):
                ...
                candidate_class = args.btad_candidate_class if args.btad_candidate_class else item
...
if __name__ == '__main__':
    ...
    parser.add_argument('--btad_candidate_class', type=str, default='')
